#!/bin/env python
# Author: Brett Savoie (bsavoie@purdue.edu)
# this is part of TAFFI program, please cite:
# J. Chem. Inf. Model. 2021, 61, 10, 5013–5027
# https://doi.org/10.1021/acs.jcim.1c00491
...
def main(argv):
    ...
    parser.add_argument('-o', dest='output_folder',type=str, default='charge_parse',
    ...
    parser.add_argument('-q', dest='qtot',type=float, default=0.0,
    ...
    parser.add_argument('-w_quad', dest='w_quadrupole',type=float, default=0.0,
    ...
    for z in [ 'AA', 'UA' ]:
        ...
        for count_f,f in enumerate(potfiles):
            ...
            if outputs[count_f] is not None:
                ...
                w_dipole = 0.0
                w_quadrupole = 0.0
                dipole_0 = array([0.0,0.0,0.0])
                quadrupole_0 = array([0.0,0.0,0.0])
            ...
            if z == "UA":
                del_ind=[ count_i for count_i,i in enumerate(adj_mat) if elements[count_i] == "H" and len([ j for count_j,j in enumerate(adj_mat[count_i]) if j == 1 and elements[count_j] == "C" and sum(adj_mat[count_j]) == 4 ]) > 0 ]
            ...
            charges_0      = array([ 0.0 if count_i in fit_ind else fixed_dict[atomtypes[count_i]] for count_i,i in enumerate(atomtypes) ])
            if len(fit_ind) == 0:
                offset = 0.0
            else:
                offset         = (args.qtot-sum(charges_0))/float(len(fit_ind))
            ...
            if args.two_step == True:
                ...
                polar_ind = [ count_i for count_i,i in enumerate(adj_mat) if \
                              ( elements[count_i] != 'C' and elements[count_i] != "H" ) or ( elements[count_i] == "H" and sum([ 1 for count_j,j in enumerate(adj_mat[count_i]) if ( j == 1 and elements[count_j] == "C" ) ]) == 0 ) ]
            ...
            lstsq_errs += [mean((Calc_Pot(seps,lstsq_charges)-potential)**(2.0))]
            fit_errs += [mean((Calc_Pot(seps,fit_charges)-potential)**(2.0))]
            ...
            # Update qtot lists           
            lstsq_qtot += [sum(lstsq_charges)] 
            fit_qtot += [sum(fit_charges)]
        ...
        for i in lstsq_dict:
            ...
            print("  {:<60s} {:35s} {:30s} {}".format(i,"{:< 8.6f} +/- {:<12.6f}".format(mean(lstsq_dict[i]),std(lstsq_dict[i])),"{:< 8.6f} +/- {:<12.6f}".format(mean(fit_dict[i]),std(fit_dict[i])),origin))

        # Print error summary
        print("\n  {:<60s} {:< 35.6g} {:< 35.6g} ".format("<xhi_pot^2> (A.U.^2):",mean(lstsq_errs),mean(fit_errs)))
        if len(lstsq_D_errs) > 0:
            print("  {:<60s} {:< 35.6g} {:< 35.6g} ".format("<xhi_D^2> (A.U.^2):",mean(lstsq_D_errs),mean(fit_D_errs)))
        print("  {:<60s} {:< 35.6f} {:< 35.6f} \n".format("total charge (A.U.):",mean(lstsq_qtot),mean(fit_qtot)))    
    ...
    write_molecule_db(args.output_folder,elements,geo,[ mean(fit_dict[i]) if i in fit_dict else 0.0 for i in atomtypes ],atomtypes)
...
def fit_charges(filename,xyz_file=None,path=None,out_file=None,qtot=0.0,gens=2,w_pot=1.0,w_qtot=1.0,w_hyper=0.0,b_hyper=0.1,w_dipole=0.1,w_quadrupole=0.0,FF_db='',N_configs=None,seed=444,\
    ...
    for z in [ 'AA', 'UA' ]:
        ...
        for count_f,f in enumerate(potfiles):
            ...
            # Generate the atomtypes
            atomtypes = id_types(elements,adj_mat,gens,Hybridization_finder(elements,adj_mat),geo*0.52917721067)
            ...
            if len(fit_ind) == 0:
                ...
                offset         = (qtot-sum(charges_0))/float(len(fit_ind))
            for i in fit_ind: charges_0[i] = offset
            ...
            if two_step == True:
                ...
                polar_ind = [ count_i for count_i,i in enumerate(adj_mat) \
                              if ( elements[count_i] != 'C' and elements[count_i] != "H" ) or ( elements[count_i] == "H" and sum([ 1 for count_j,j in enumerate(adj_mat[count_i]) if ( j == 1 and elements[count_j] == "C" ) ]) == 0 ) ]
    ...
    charges = { i:(mean(fit_dict[i]),std(fit_dict[i])) for i in fit_dict }
    errs    = { "xhi_pot":mean(fit_errs) }
    if len(fit_D_errs) > 0:
        errs["xhi_D"] = mean(fit_D_errs)
...
def flex_fit(geo,potential,seps,charge_mapping=None,fit_indices=None,\
    ...
    if charges_0 is None:
        charges = zeros(len(geo)) + qtot/float(len(geo))
    else:
    ...
    # charges -= (sum(charges) - qtot)/float(len(charges)) # OLD, new only subtracts from fit_ind
    charges[fit_indices] -= (sum(charges) - qtot)/float(len(fit_indices))
    return charges
...
def flex_fit_func(q_0=None, q_1=None, q_2=None, q_3=None, q_4=None, q_5=None, q_6=None, q_7=None, q_8=None, q_9=None,q_10=None,\
    ...
    fit_charges = array([ local_vars[j] for j in natural_sort([ i for i in local_vars if 'q_' in i and local_vars[i] is not None ]) ])
    ...
    return w_pot * mean(( potential - dot(seps,charges.T) )**(2.0)) + w_hyper * mean( (charges**(2.0) + b_hyper**(2.0) )**(0.5) - b_hyper ) + w_qtot*(sum(charges)-qtot)**(2.0) +\
           w_dipole*mean((calc_dipole(geo,charges,r_0=r_0)[0] - dipole_0)**(2.0)) + w_quadrupole*mean(((calc_quadrupole(geo,charges,r_0=r_0,traceless=traceless_opt) - quadrupole_0 )**(2.0)).flat)
...
def calc_cm(elements,geo):
    ...
    if set(elements).issubset(list(Masses.keys())) == False:
        ...
        masses = array([ Masses[i] for i in elements ])
...
def calc_quadrupole(geo,charges,r_0=None,traceless=True):
    ...
    if traceless == True:        
        ...
        for i in range(3):
            for j in range(3):
                if i == j:
                    Q[i,j] = sum((3.0*geo[:,i]*geo[:,j]-geo_norms)*charges)
                else:
                    Q[i,j] = sum(charges*(3.0*geo[:,i]*geo[:,j]))
    ...
    with open(filename,'r') as f:
        ...
        for lc,lines in enumerate(f):
            ...
            if lc == 0: 
                ...
                geo       = zeros([n_atom,3])
                grid      = zeros([n_grid,3])
                potential = zeros([n_grid])
                flag = 1
                continue

            # Parse the grid and potential
            if flag == 2:
                grid[pot_count] = array([ float(i) for i in fields[1:4] ])
            ...
            if flag == 1:
                ...
                if xyz_flag == 0:
                    ...
                    geo[atom_count]      = array([ float(i) for i in fields[1:4] ])
                atom_count += 1
            ...
            for lc,lines in enumerate(f):
                ...
                if lc > 1:
                    ...
                    if len(fields) == 0: 
                        ...
                        N_geo[atom_count] = array([ float(i)/0.52917721067 for i in fields[1:4] ])
                        atom_count += 1
    ...
    with open(output_file,'r') as f:
        ...
        for lines in f:
            ...
            if len(fields) == 7 and fields[0] == "Total" and fields[1] == "Dipole" and fields[2] == "Moment":
                dipole_0 = array([ float(i) for i in fields[4:7] ])

            # quadrupole parsing commands
            if quad_flag == 6:
                quadrupole_0      = zeros([3,3])
...
def lstsq_potfit(geo,grid,q_tot,seps,potential):
    ...
    A = vstack([seps,ones(len(geo))*float(len(grid))])
    b = hstack([potential,array([q_tot * float(len(grid))])])

    # Calculate inverse
    A_inv = linalg.pinv(A)

    # Calculate fit coefficients                                                                                                                                                                                                             
    charges_lstsq = dot(A_inv,b)
    charges_lstsq = charges_lstsq - ( sum(charges_lstsq) - q_tot ) / float(len(charges_lstsq))
...
def write_params(name,charge_dict):
    ...
    with open(name,'w') as f:
        ...
        for i in natural_sort(list(charge_dict.keys())):
            if '-UA' not in i:
                f.write("{:10s} {:40s} {:< 20.6f}\n".format("charge",str(i),mean(charge_dict[i])))
        for i in natural_sort(list(charge_dict.keys())):
            if '-UA' in i:
                f.write("{:10s} {:40s} {:< 20.6f}\n".format("charge",str(i),mean(charge_dict[i])))
